...
namespace LMY.MSWordEditor
{
    public class LMYMSWordEditorMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext)
        {
            try
            {
                    ...
                    httpContext.Request.Headers.Any(x => x.Value[0].ToLower().Contains("microsoft office")))
                {
                    ...
                    { 
                        long contentLength = GetFileBytes(httpContext).Length;
                    ...
                    else if (httpContext.Request.Method == "GET")
                    {
                        ...
                        var fileContents = GetFileBytes(httpContext);
                    ...
                    else if (httpContext.Request.Method == "PUT")
                    {
                        ...
                        using (MemoryStream memoryStream = new MemoryStream())
                        {
                            ...
                            // Now 'memoryStream' contains the byte array
                            byte[] byteArray = memoryStream.ToArray();

                            SaveFileBytes(httpContext, byteArray);
                        }
        ...
        private byte[] GetFileBytes(HttpContext httpContext)
        {
            if (_options.OnGetFile != null)
            {
                return _options.OnGetFile(GetTokenFromUrl(httpContext.Request.Path), GetFilePath(httpContext.Request.Path), httpContext);
            }
            return null;
        }

        private bool SaveFileBytes(HttpContext httpContext, byte[] fileBytes)
        {
            if (_options.OnGetFile != null)
            {
                return _options.OnSaveFile(GetTokenFromUrl(httpContext.Request.Path), GetFilePath(httpContext.Request.Path), fileBytes, httpContext);
            }
            return true;
        }
        ...
        {
            string filePath = Path.Combine(ClearAndRemoveTokenUrl(path.ToString()));
